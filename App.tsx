...
const DEFAULT_SETTINGS: GlobalSettings = {
  ...
  schoolAddress: "P.O. BOX 123, LOCATION, ACCRA", // Added Default Address
  ...
  academicCalendar: {},
  calendarLists: {
      periods: [], // Will be populated from constants if empty
      activities: [],
      assessments: [],
      leadTeam: [],
      extraCurricular: []
...
const App: React.FC = () => {
  ...
  const [activeModule, setActiveModule] = useState<Module>("Result Entry"); // Changed default to Result Entry since Assessment might be gone
  ...
  const getGenericModuleName = () => {
      ...
      if (activeModule === 'Time Table') {
          if (timetableSubTab === 'class_timetable') {
              // Renamed "Observation Schedule" to "Preschool and Kindergarten Time Table" for Daycare/Early Childhood
              return isObservationDept ? 'Preschool and Kindergarten Time Table' as any : 'Class Time Table' as any;
          }
          if (timetableSubTab === 'exam_timetable') {
              // This sub-tab in Daycare/Kindergarten becomes "Observation Schedule"
              // In Basic/JHS it remains "Examination Time Table"
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans flex flex-col">
      ...
      {/* 4. Sub-Module Navigation (Assessment Removed from MODULES constant but activeModule check kept for safety) */}
      ...
      {/* 4b. Sub-Module Navigation for Time Table (Time Table Removed from MODULES constant but activeModule check kept for safety) */}
      {activeModule === 'Time Table' && (
          <div className="no-print bg-gray-50 border-b border-gray-200 px-4 py-2 flex gap-4 justify-center flex-wrap">
             ...
             >
                {isObservationDept ? 'Preschool and Kindergarten Time Table' : 'Class Time Table'}
             </button>
             
             {/* Show Observation Schedule for Daycare OR Exam Timetable for Basic/JHS */}
             <button
